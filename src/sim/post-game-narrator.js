...
function buildNarratorUserPrompt({ outcome, turnsCompleted, finalState, turns, collapse }) {
...
function buildHandBuiltSummary({ outcome, turnsCompleted, finalState, collapse, turns }) {
  ...
  const lastMove = turns.length > 0 ? turns[turns.length - 1].playerMove : '(no moves)';
  const narrative = `You governed for ${turnsPlayed} turn${turnsPlayed === 1 ? '' : 's'}. Your final move was: "${lastMove.slice(0, 200)}${lastMove.length > 200 ? '...' : ''}".`;
...
async function narrateRunEnd({ outcome, turnsCompleted, finalState, turns, collapse, model = process.env.OPENROUTER_MODEL, maxAttempts = 2 } = {}) {
  loadEnv(ROOT_DIR);

  const systemPrompt = buildNarratorSystemPrompt();
  const userPrompt = buildNarratorUserPrompt({ outcome, turnsCompleted, finalState, turns, collapse });
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    try {
      ...
      return { ...buildHandBuiltSummary({ outcome, turnsCompleted, finalState, collapse, turns }), fallback: true };
    }
