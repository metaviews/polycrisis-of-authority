...
async function runInteractive({ maxTurns = MAX_TURNS, model = process.env.OPENROUTER_MODEL || 'minimax/minimax-m3' } = {}) {
  ...
  console.log('  End your move with a blank line. The session ends on collapse,');
  console.log('  when the regime stabilizes, or after ' + maxTurns + ' turns.');
  console.log('');
  ...
  try {
    while (turn < maxTurns) {
      ...
      if (outcome === 'stabilized') {
        ...
        console.log('  The simulation ends — there is no further crisis to address.');
        break;
      }
    }
