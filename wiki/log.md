...
- **Action:** Filed the v1 interactive web surface. 6 routes (4 GET from v0 + 2 new POST). The two new routes are `POST /runs` (start a new run) and `POST /runs/:id/move` (submit a move). Each turn is a separate HTTP request, with state persisted to `data/runs/<id>.json` between requests.
- **Engine change:** `src/sim/run-loop.js` gained two new exported functions — `stepTurn` and `pickCrisis`. The change is *additive*: no logic changes, no behavior changes to the existing `runLoop`. The discord bot and terminal version are unaffected.
- **Why the engine change was needed:** the spec said "no changes to `src/sim/`" but the engine's only public entry points (`runLoop`, `runSimulationAsync`) didn't fit an HTTP surface where each request is a separate process invocation. `stepTurn` is the per-turn API the v1 surface needs. User confirmed path 1 (surgical extraction) over path 2 (child-process model) or path 3 (long-lived in-memory process).
- **Auth decision:** the v1 surface ships *without* bearer-token auth. The run URL is the access — the player bookmarks their run URL and comes back to it. Reasons: project's no-PII posture, user's gating of `npm install` (no sqlite or auth-cookie libraries), and the run URL was already the access model in v0. v2 (cycle 13+) layers auth on top.
- **The v1 routes:** `GET /`, `GET /runs`, `GET /runs/:id`, `GET /runs/:id/report` (carried over from v0). `POST /runs` (new), `POST /runs/:id/move` (new). `POST /runs/nonexistent/move` returns 404, `POST` on a seed run returns 404 (only live runs accept moves), empty move body returns 400.
- **End-of-run flow:** when `stepTurn` returns a `collapse` or the state stabilizes (5 consecutive turns in `holding` or `strained`), the v1 server calls `narrateRunEnd` (LLM-driven end-of-run narrative), `generateArtifact` (the post-game markdown), and `renderArtifactHtml` (self-contained HTML). The artifact HTML is written to `data/runs/<id>-artifact.html`. The run URL is the share — same URL, state-driven content.
- **Verification:** 26 of 26 ad-hoc checks pass at `/tmp/hermes-verify-12c-v1.sh`. Categories: engine exports (stepTurn, pickCrisis, runLoop), in-process pickCrisis for turns 1 and 2+, stepTurn with a mock LLM, v1 server boots and serves 4 GET routes, POST /runs starts a new run, POST /runs/:id/move advances the turn, state persists across requests (resumability), error cases (404, 400), stepTurn with real generateWorld (engine integration intact), no behavior change to runLoop. Real LLM calls were made during verification — a run was played end-to-end.
- **Real-run confirmation:** the run `20260727180033-09q34e` was played during verification. 2 turns, both with real LLM-generated situation/pressure/decision-point prose, both with the player's moves quoted in the chat-thread layout, both with grounding traces linked to wiki pages. The state evolved (legitimacy: 65 → 70 after the player's "publish the leaked draft" move).
- **Filed:** `wiki/prototypes/2026-07-27-cycle-12c-v1.md`.
- **Next:** user plays the v1 surface. After a few playthroughs, cycle 12d starts: corpus-quote footer + /status + post-game report integration. The v1 surface becomes feature-complete. v2 (cycle 13+) is the auth layer + multi-player.

## 2026-07-28 — Web v1 surface complete (cycle 12d)

- **Action:** Filed the cycle 12d changes that finish the v1 surface. The v0 read-only surface (cycle 12b) had 4 GET routes. The v1 interactive surface (cycle 12c) added 2 POST routes. Cycle 12d adds the `/status` page, the deliberate corpus-quote picker, and the wired decision-dock form. The v1 surface is now feature-complete per the spec.
- **The four pieces:**
  1. **`/status` page** — `GET /runs/:id/status` returns a new page showing the 6 axes, their values, their bands, and a back-link. Per spec decision 4, this is the *only* place the system becomes visible. Austere: no charts, no sparklines, no gradients. The `bandFor` helper (cycle 12d adds it to the surface adapter) mirrors the engine's `bandFor` (collapsed/eroded/strained/holding bands at 0/50/60/80 thresholds). Verified at all 8 boundary points.
  2. **deliberate corpus-quote picker** — the v1 surface (cycle 12c) was rendering corpus quotes by deriving them from the LLM's `grounding_trace[0]`. Cycle 12d replaces that with the project's deliberate picker: `pickCorpusQuote(preferHref)` from `scripts/wiki-query.js` (cycle 5e). The picker is called at *display time* with `preferHref` set to the *prior turn's* grounding trace — creating a chain: turn 1's quote is random, turn 2's points to what grounded turn 1, etc. The player can browse the corpus as a knowledge-graph traversal of the run.
  3. **wired decision-dock form** — the v1 surface (cycle 12c) was *half-wired*: the server accepted `POST /runs/:id/move`, but the form in the rendered page was still the v0 placeholder (textarea and button both `disabled`, with a note saying "v0 is read-only"). Cycle 12d wires the form: the textarea is no longer disabled, the submit button works, a small inline `<script>` defines `polycrisisSubmitMove(event, runId)` that strips trailing blank lines, fetches the move as JSON, replaces the document with the server's response, and scrolls to the bottom. The "End your move with a blank line" hint matches the cycle 5b.5 behavior. **This is the load-bearing change of cycle 12d** — without it, the v1 surface was a server that no client could talk to.
  4. **seed run state fields** — the seed runs in `data/seed-runs/` were authored for the v0 surface that didn't need state. The `/status` page needs `state` on every run. Cycle 12d adds the `state` field to both seed runs with hand-crafted values that match the outcome (`no-collapse`: mostly `strained`/`eroded` bands; `collapse`: multiple `collapsed` bands). Real v1 runs (in `data/runs/`) get `state` automatically — the session file persists state across requests.
- **No engine changes.** Cycle 12d is a *web-only* cycle. The `bandFor` helper in the surface adapter is a *display* concern, not a *simulation* concern. The engine is unchanged.
- **Verification:** 35 of 35 ad-hoc checks pass at `/tmp/hermes-verify-12d-status.sh`. Categories: surface exports, bandFor boundary matching, renderStatusPage content, pickQuoteForTurn wiring, decision-dock form wiring, v1 server boots and serves 5 GET routes, /status page has axes/bands/back-link/"System status" h1, austere aesthetic, collapse run's /status shows multiple collapsed bands, /status 404 for non-existent runs, POST /runs returns the wired form, v0 placeholder gone, textarea/button no longer disabled, POST /runs/:id/move advances the turn (real LLM call), after-move page has corpus-inline panel with wiki link, /status on a live v1 run shows the evolved state, seed runs have state fields, aesthetic compliance. Real LLM calls were made during verification — the v1 surface was driven end-to-end.
- **Filed:** `wiki/prototypes/2026-07-28-cycle-12d-complete.md`.
- **What 12d does NOT do:** no advisor interaction (the buttons in the advisor strip are still non-functional — 12d+1 or a separate cycle), no /status for the cold-start itself (correct — /status is per-run, not global), no authentication (v2), no post-game report content changes (cycle 12c already does this).
- **Next:** user plays the v1 surface. The v1 surface is feature-complete per the spec. v2 (cycle 13+) is the auth layer + multi-player. The v0/v1 surface contract is documented in `src/web/README.md`; v2 can layer auth on top without changing the surface contract.

## 2026-07-28 — Web advisor interaction (cycle 12d+1)

- **Action:** Filed the cycle 12d+1 changes that wire the advisor panel to the engine's `consult()` function. The v1 surface (cycles 12b/12c/12d) had the advisor panel *rendered but non-functional* — five buttons, no click handler, no read, no LLM call. Cycle 12d+1 wires the panel to the engine's corpus-grounded advisor, the same function the discord bot has been using since cycle 6.
- **The four pieces:**
  1. **the advisor panel** — `renderDecisionDock` (which already had the form for move submission) now also renders an advisor panel below the form. 5 buttons in a row, one per voice (frontier-lab, civil-society, state-security, open-source, international-ally). A read area below the buttons, initially empty with a hint. Each button has a `data-voice` attribute and an `onclick` that calls the inline JS function `polycrisisConsultAdvisor(runId, voice)`. The active button is marked `data-active="true"`.
  2. **the server route** — `POST /runs/:id/advisor`: validates the voice is one of the 5 ADVISOR_VOICES, checks the cache, calls `consult({ voice, crisis, state, playerMove: '[player is consulting before writing their move]', identity })` if not cached, returns `{ voice, read, retrievedPages, fromCache }` as JSON. The LLM call is ~5-10s; cache hit is <10ms.
  3. **the read area** — empty state (italic hint about consulting) or populated (voice label, read body in serif prose, "Sources:" line with corpus page links). Re-clicks show "(cached)" tag. The engine's `consult()` retrieves 6 corpus pages (4 voice-specific + 2 crisis-relevant) and feeds them to the LLM with a constrained prompt. The result is *grounded in the project's own corpus*, not just LLM hallucination.
  4. **the bug fix: turn-1 dock** — cycle 12d+1 also fixes a pre-existing bug in the v1 surface: the decision dock was *not rendering on turn 1* because the v1 server's `handleRunPage` and `handleSubmitMove` only included *prior* turns in the surface adapter's `turns[]` array. The current crisis (which is what the player is reasoning about RIGHT NOW) wasn't included. The fix: for active runs, push the `currentCrisis` as the last item in `surfaceTurns` (with `player_move: null`). The surface adapter then has the current crisis in its data, and the dock renders. This was a *pre-existing* gap; the advisor panel lives in the dock, so the bug fix is part of 12d+1.
- **The cache model.** session.advisorReads is keyed by `${currentTurn}:${voice}`. When the player advances to turn 2, the turn-1 consults are *not* shown anywhere (they're in the session but the surface adapter only shows the current turn's read). This matches the discord model: advisor consults are transient interactions, not part of the run record. A re-click of the same voice on the same turn returns the cached read in <10ms with `fromCache: true`.
- **The read area persistence.** When the player consults an advisor and then refreshes the page, the consult is preserved — the session has `advisorReads`, the page re-renders with `deriveAdvisorRead(session)`, and the read area is populated. The most recent consult is shown; the prior turn's consults are not displayed.
- **No engine changes.** Cycle 12d+1 is a *web-only* cycle that uses the existing `consult()` function from `src/sim/advisors.js`. The engine is unchanged.
- **Verification:** 28 of 28 ad-hoc checks pass at `/tmp/hermes-verify-12dplus-advisors.sh`. Categories: surface adapter signature (advisorRead + consultedVoice), 5 advisor voices rendered, empty/populated read states, server imports consult+ADVISOR_VOICES from engine, handleConsultAdvisor + route wired, deriveAdvisorRead + deriveConsultedVoice helpers, advisor CSS, live HTTP test (POST /runs + POST /advisor + cache + error cases + page re-render + (cached) tag), aesthetic compliance. Real LLM calls were made during verification. The frontier-lab consult returned a 932-char read grounded in 6 corpus pages; the civil-society consult returned a 1033-char read grounded in Algorithmic Authority, Algorithmic Transparency, OpenAI and Anthropic, Agentic AI, and two signal entries.
- **Filed:** `wiki/prototypes/2026-07-28-cycle-12d-plus-advisors.md`.
- **What 12d+1 does NOT do:** no per-turn advisor history in the chat-thread (matches the discord model — advisor consults are transient), no "refresh" button (re-click returns the cached read; v2 could add this), no rate limit (a player could click all 5 voices 5 times each, triggering up to 25 LLM calls per turn — cost is the player's choice), no turn-1 placeholder fix (the seed crisis has `pressure: "(LLM-generated)"` and `decision_point: "(LLM-generated)"` as placeholders — on turn 1, the decision question shows this placeholder; a real question appears on turn 2+; this is a follow-up issue).
- **Next:** the v1 surface is now feature-complete per the spec: 5 decisions (auth, corpus, post-game, trajectory, advisor) all landed. v2 (cycle 13+) is the auth layer + multi-player. Cycle 12e follow-up: turn-1 placeholder question. The cycle 6g thread (discord polish + deployment) is still open.

## 2026-07-28 — Turn-1 placeholder decision question (cycle 12e)

- **Action:** Filed the cycle 12e fix for a UX gap in the v1 surface: on turn 1, the seed crisis has placeholder `pressure: "(LLM-generated)"` and `decision_point: "(LLM-generated)"` strings. The engine elaborates these after the player's first move, but on turn 1 the player never sees the elaboration — they'd see the literal placeholder text in the decision dock and the per-turn card. Cycle 12e adds per-failure-pattern question and pressure substitution to the surface adapter.
- **The four failure patterns and their questions:**
  ...
  - (unknown pattern) → "How do you respond to this situation?"
- **Each pattern also has a per-pattern pressure** (1-2 sentences, more specific than the question). The pressure explains *why* the question is being asked.
- **The substitution logic.** Two new helpers: `resolveDecisionQuestion(decisionQuestion, failurePattern)` and `resolveDecisionPressure(pressure, failurePattern)`. Both check if the engine produced a real (non-placeholder) string; if so, pass through. If it's the placeholder, substitute the per-pattern value. If the pattern is unknown, fall back to a generic. Both are case-insensitive on the placeholder match.
- **Where the substitution happens.** Two places in the surface adapter: `renderTurnCard` (per-turn card in the chat-thread — prior turns get the same treatment, so a player going back to read turn 1 sees a real pressure and question) and `renderDecisionDock` (the decision dock at the bottom of the page).
- **No engine changes.** Cycle 12e is a *display* fix in the surface adapter, not a simulation change. The engine still produces the placeholders; the LLM still elaborates them after the first move.
- **Why not call the LLM at run-start?** Considered but rejected: an LLM call on every run-start, even before the player has written, would slow the player's first impression. The per-pattern question approach has the LLM elaborate after the first move (which is when the world generator runs anyway), with 0 extra LLM calls for the turn-1 substitution.
- **Verification:** 17 of 17 ad-hoc checks pass at `/tmp/hermes-verify-12e-placeholder.sh`. Categories: per-pattern maps defined, resolveDecisionQuestion (real passes through, 4 placeholder+pattern combinations substitute, case-insensitive, unknown falls back), resolveDecisionPressure (same logic), renderDecisionDock and renderTurnCard use the resolvers, placeholder text doesn't appear in the rendered page, v0/v1 regressions. The existing live run from cycle 12d+1 testing (failure_pattern: legitimacy-erosion) was rendered with the per-pattern question and per-pattern pressure. Zero matches for `(LLM-generated)` in the rendered page.
- **Filed:** `wiki/prototypes/2026-07-28-cycle-12e-placeholder.md`.
- **What 12e does NOT do:** no LLM call at run-start, no engine changes, no per-pattern situations (the seed's `trigger` is the real situation, not a placeholder), no per-pattern headlines (headlines are post-move world generator output).
- **Next:** the v1 surface is now feature-complete per the spec. v2 (cycle 13+) is the auth layer + multi-player. Cycle 6g (discord polish) is still open.

## 2026-08-01 — Web opening title integration (cycle 13)

- **Action:** The opening title sequence (`assets/videos/prototype-2026-08/polycrisis-of-authority-opening.mp4`, ~12.5s, 2K, ~16.7MB, committed in git as part of the surface) shipped in the August 2026 interstitial-video prototype 5 cycle but had no consumer. Cycle 13 wires it into the web surface at the boundary moments of the run: cold-start (`GET /`) and end-of-run (`GET /runs/:id` when `state='ended'`). Not on active runs (the player is mid-play, not arriving) and not on `/status` (subordinate to the run page).
- **Why now.** Per Principle 4.4 ("public surfaces wait"), the engine has been stable for a cycle. The web v1 surface is feature-complete since cycle 12e. This is the first v1.1 polish cycle — the natural next adornment, small and isolated.
- **Surface adapter change (additive).** New helper `renderOpeningTitle()` in `src/web/surface.js`. Emits an inline `<video autoplay muted playsinline loop>` with the prototype asset as its `src`, plus a small mono uppercase "SKIP" link wired to an inline `polycrisisSkipOpening()` that removes the element on click. The helper carries its own `<style>` block inside the element (rather than threading through `renderPageShell`'s CSS), matching the precedent set by cycles 12c-12e inline style blocks.
- **`renderPageShell` change (small).** Added an optional `preMeta` field. When present, it is rendered *before* `runMeta` inside `<div class="page">`, so the opening title becomes the first chrome read on the page (above the run-meta header). `renderRunPage` passes `preMeta = !isActive ? renderOpeningTitle() : ''` — ended runs only. `renderColdStart` passes `preMeta: renderOpeningTitle()` unconditionally. No other call site of `renderPageShell` is affected.
- **Server change (additive).** New route `GET /assets/videos/...` with `handleAssets` handler. Whitelist-only: only serves files under `ROOT_DIR/assets/` that exist on disk and end in `.mp4`. Path traversal attempts (`..`, absolute paths, deep traversal) are rejected with 404 *before* any `fs` call. Defense in depth: `rel.includes('..')` check + `path.resolve` check that the resolved path is still under `ASSETS_ROOT` + ext whitelist + existence + isFile check. Content-type `video/mp4`. Cache-control `public, max-age=3600`.
- **No engine changes.** Cycle 13 is *display + route* only. `src/sim/` is unchanged. The cycle 12c engine extraction (`stepTurn`, `pickCrisis`) is unaffected.
- **Audio stance in the embed.** The video is `muted` in the embed. The project's "library quiet" feel doesn't surprise the player with audio on first paint. The source MP4 has ambient audio (faint wind, distant water, brush on paper) — available if the stance is revisited.
- **Verification:** 25 of 25 ad-hoc checks pass at `/tmp/hermes-verify-13-opening.sh`. Categories: file existence (asset + helper + route), no engine coupling, surface adapter renders correctly (cold-start has video before h1, active run has NO video, ended run has video above run-meta, status page has NO video), skip link present + visible from start + wired to `polycrisisSkipOpening`, aesthetic compliance (mono uppercase skip, no gradients/shadows, `--rule`/`--muted` in use), static asset route (200 + `video/mp4` + size matches disk) + path traversal protection (`/assets/../etc/passwd` → 404, deep traversal → 404, non-`.mp4` extension → 404, missing file → 404, non-existent dir → 404), live HTTP (cold-start HTML contains opening.mp4 + skip link, both seed runs' run pages contain opening.mp4, status pages do NOT, `/runs` JSON does NOT), no regressions (cycle 12d/12d+1/12e exports still work, `bandFor` thresholds, `resolveDecisionQuestion` substitution).
- **Visual confirmation:** headless chromium render of the cold-start page at 1280×900 (`/tmp/13-cold-start.png`) shows the video element at the top with the title text "polycrisis of authority" mid-frame on the parchment background, the small uppercase "SKIP" link in the top-right corner of the video, the `--rule` border-bottom separating the video from the page content below, and the standard `h1` + "v1 surface · interactive" + frame paragraph rendering as expected. The austere mono/serif register holds.
- **Filed:** `wiki/prototypes/2026-08-01-cycle-13-opening-title-integration.md`. Surface adapter README updated. `docs/26-interstitial-video.md` sequence step 5 closed.
- **What 13 does NOT do:** does not play the video between turns (still [open] in the interstitial doc), does not generate new video, does not change the audio stance unless revisited, does not add bearer-token auth (still v2 / cycle 14+), does not change the engine.
- **Next:** the opening title now has a consumer on the web surface. The remaining open surface work is cycle 6g (discord polish, paused) and the v2 web auth + multi-player (cycle 14+). Between-turns interstitials direction is still [open] — when revisited, the cycle can reuse `renderOpeningTitle` as a precedent for the between-turns helper.
