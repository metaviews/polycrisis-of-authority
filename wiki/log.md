...
- **Action:** The opening title sequence (`assets/videos/prototype-2026-08/polycrisis-of-authority-opening.mp4`, ~12.5s, 2K, ~16.7MB, gitignored) shipped in the August 2026 interstitial-video prototype 5 cycle but had no consumer. Cycle 13 wires it into the web surface at the boundary moments of the run: cold-start (`GET /`) and end-of-run (`GET /runs/:id` when `state='ended'`). Not on active runs (the player is mid-play, not arriving) and not on `/status` (subordinate to the run page).
